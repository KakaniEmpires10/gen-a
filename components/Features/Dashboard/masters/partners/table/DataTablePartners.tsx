...
import { Search, RefreshCw, Handshake, AlertCircleIcon } from "lucide-react";
...
export function DataTablePartners<TData extends Partners, TValue>({
    columns,
}: DataTableProps<TData, TValue>) {
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
    const [open, setOpen] = useState<boolean>(false);

    const { data, isLoading, error, isValidating } = useSWR('/api/partners');
        ...
        <>
            <div className="flex items-center justify-between py-4 border-y border-y-primary">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={handleOpenDialog}>
                        <Handshake className="size-4" /> Tambah Mitra
                    </Button>
                </div>
            </div>
            ...
            <div className="relative mt-8">
                <Alert className="absolute -top-10 left-0 right-0 w-[95%] m-auto z-[5]">
                    ...
                    <AlertDescription>
                        Disini nih buat lihat daftar Mitra &nbsp;
